  ...
  static inline double dynamic(const vector<double> &v, const vector<double> &w,
    ...
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < n; ++j) {
        ...
        if (i > 0)
          previous.push_back(gamma[i - 1][j]);
        if (j > 0)
          previous.push_back(gamma[i][j - 1]);
        if ((i > 0) && (j > 0))
...
public:
  NaiveNearestNeighbor(const vector<double> &v, int constraint)
      : NearestNeighbor(v, constraint), lb_keogh(0), full_dtw(0), V(v),
        bestsofar(dtw::INF) {}
...
public:
  LB_Keogh(const vector<double> &v, int constraint)
      : NearestNeighbor(v, constraint), lb_keogh(0), full_dtw(0), V(v),
        mConstraint(constraint), bestsofar(dtw::INF), U(V.size(), 0),
        L(V.size(), 0) {
...
public:
  LB_Improved(const vector<double> &v, int constraint)
      : NearestNeighbor(v, constraint), lb_keogh(0), full_dtw(0), V(v),
        buffer(v), mConstraint(constraint), bestsofar(dtw::INF), U(V.size(), 0),
        L(V.size(), 0), U2(V.size(), 0), L2(V.size(), 0) {
    assert(mConstraint >= 0);
    assert(mConstraint < static_cast<int>(V.size()));
    computeEnvelope(V, mConstraint, U, L);
  }
...
public:
  DimReducedLB_Keogh(const vector<double> &v, int constraint, int reduced)
      : NearestNeighbor(v, constraint), lb_keogh(0), full_dtw(0), V(v),
        mConstraint(constraint), bestsofar(dtw::INF), U(V.size(), 0),
        L(V.size(), 0), Ured(reduced, 0), Lred(reduced) {
    assert(mConstraint >= 0);
    assert(mConstraint < static_cast<int>(V.size()));
    computeEnvelope(V, mConstraint, U, L);
